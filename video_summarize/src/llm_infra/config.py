...
1. **模型/阶段参数**（原 llm_profiles）：三层配置解析，后层字段级覆盖前层
   ...
   max_tokens）由本模块翻译成各家真实参数形态（deepseek 的 thinking:{type}、
   ...
   配置来源（后者字段级覆盖前者）：
   - video_summarize/config/llm_profiles.json（纳入版本控制）
   - video_summarize/config_local/llm_profiles.json（本地覆盖，可选）
...
2. **管线批大小/阈值**（原 parameter_config）：管「一次调用处理多少个工作单元
   ...
   配置来源（字段级覆盖，后者优先）：
   - video_summarize/config/parameter_config.json（纳入版本控制）
   - video_summarize/config_local/parameter_config.json（本地覆盖，可选）
...
_PROFILES_CONFIG_PATH = Path(__file__).parent.parent.parent / "config" / "llm_profiles.json"
_PROFILES_LOCAL_CONFIG_PATH = (
    Path(__file__).parent.parent.parent / "config_local" / "llm_profiles.json"
)
...
def load_config() -> dict[str, Any]:
    """加载仓库配置 + 本地覆盖，合并为一份配置 dict。"""
    config: dict[str, Any] = {"default_profile": "deepseek-v4-flash", "profiles": {}, "stages": {}}
    for path in (_PROFILES_CONFIG_PATH, _PROFILES_LOCAL_CONFIG_PATH):
        if not path.exists():
            continue
        patch = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(patch, dict):
            raise ValueError(f"{path} 内容不是 JSON 对象: {type(patch).__name__}")
...
# ═══════════════════════════════════════════════════════════════════════════
# 二、管线批大小/分组/embedding 阈值（parameter_config.json）
# ═══════════════════════════════════════════════════════════════════════════

_PARAM_CONFIG_PATH = Path(__file__).parent.parent.parent / "config" / "parameter_config.json"
_PARAM_LOCAL_CONFIG_PATH = (
    Path(__file__).parent.parent.parent / "config_local" / "parameter_config.json"
)
...
def load_parameter_config() -> dict[str, Any]:
    ...
    for path in (_PARAM_CONFIG_PATH, _PARAM_LOCAL_CONFIG_PATH):
        if not path.exists():
            continue
        try:
            patch = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            logger.warning("参数配置文件 %s 损坏，跳过（使用内置默认值）", path)
            continue
        if not isinstance(patch, dict):
            logger.warning("参数配置文件 %s 内容不是 JSON 对象，跳过", path)
            continue
