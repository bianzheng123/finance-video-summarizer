...
from .rendering.rendering_bilibili import BilibiliRenderer as BilibiliRendererImpl
...
from .rendering.upload_trimming import (
    FIXED_SECTIONS as FIXED_SECTION_NAMES,
    UPLOAD_CHAR_LIMIT,
    trim_for_upload,
    visible_text,
)
...
class SummaryRenderer:
    """总结文件渲染主控制器：生成 summary.md + 结构化 HTML + 公众号 HTML + B站版 Markdown + 表格图片。"""
    ...
    def _build_parts(
        ...
        bvid: str | None,
        rendering_dir: Path,
        bili_impl: BilibiliRendererImpl | None,
        *,
        economic: bool = False,
    ) -> dict[str, str]:
        """按渲染顺序一次遍历，产出每个渲染器的完整文本，返回 {渲染器名: 文本}。

        bili_impl 为 None 时不生成 TLDR 表格图片（量字数的试渲染走这条路，纯字符串无 IO）。
        """
        ...
        for section_key in self._get_render_order(analysis):
            ...
            if section_key == "精炼总结":
                self._render_tldr_section_parallel(
                    renderers, renderer_parts, section_data, rendering_dir, bili_impl,
                    economic=economic,
                )
    ...
    def render_save(self, output_dir: Path, analysis: dict, bvid: str | None = None, author: str | None = None, title: str | None = None, *, economic: bool = False) -> str:
        """生成所有格式的总结文件，返回完整版 Markdown 内容。

        两趟渲染：
        - Pass A 完整版（不裁剪）→ summary.md / summary.html / summary.pdf / summary_structured.html
        - Pass B 上传版（阶梯裁剪到 UPLOAD_CHAR_LIMIT 以内）→ summary_gongzhonghao.html /
          summary_bilibili.md + TLDR 表格图；裁剪后的 analysis 落 llm_analysis_upload.json
        """
        with step_logger("渲染"):
            ...
            # 裁剪基线：清理后、注入精炼总结前的干净副本。精炼总结在每次量字数前按
            # 当前裁剪结果重算，故基线里不能留旧 TLDR。
            trim_base = deepcopy(analysis)
            trim_base.pop("精炼总结", None)
            ...
            # B 站版始终渲染：投稿失败（如账号封禁）时 bvid 为 None，正文照常产出，
            # 只是时间戳不带跳转链接——专栏草稿仍可保存，解封后再补链接。
            bili_impl = BilibiliRendererImpl(bvid, unresolved_jargons)

            # ========== Pass A：完整版 ==========
            full_parts = self._build_parts(
                analysis,
                [('summary', summary_impl), ('gzh', gzh_impl)],
                author, title, bvid, rendering_dir,
                bili_impl=None,  # 表格图只由上传版生成，与 summary_bilibili.md 同源
                economic=economic,
            )
            ...
            # ========== 渐进式裁剪 ==========
            # 量字数口径 = **上传版公众号 HTML** 剥离标记后的可见文字数，即
            # 与落盘的 summary_gongzhonghao.html 完全同一份字节的可见文字。
            #
            # 两个都别省：
            # - 必须剥标记：公众号渲染器为控制样式会吐出大量内联 HTML 表格骨架
            #   （`<table style=...>` / `<colgroup>` / 每格 `<td style=...>`），实测
            #   原始长度虚高 1.42 倍；更糟的是骨架大小随速览表行数变化，等于给
            #   速览表偷偷加双倍权重，会导致裁剪过度。
            # - 必须先转 HTML 再量：markdown 阶段的可见文字比转换后多 2.2%
            #   （markdown 的段落空行等在 HTML 里被折叠），量 markdown 等于量了一份
            #   并不存在的产物。转换约 16~28 ms，乘单元池几十个仍在一秒内。
            #
            # 选公众号版而非 B 站版：它以文字表格而非图片链接呈现速览表，可见文字
            # 更多，对两个平台都是保守估计。
            def measure(candidate: dict) -> int:
                probe = self._prepare_analysis(deepcopy(candidate))
                parts = self._build_parts(
                    probe, [('gzh', gzh_impl)],
                    author, title, bvid, rendering_dir, bili_impl=None,
                    economic=economic,
                )
                html_text = GongzhonghaoRendererImpl.markdown_to_html(parts['gzh'])
                return len(visible_text(html_text))

            trimmed, dropped_units, upload_chars = trim_for_upload(trim_base, measure)
            trimmed = self._prepare_analysis(trimmed)

            upload_json = output_dir / ("llm_analysis_economic" if economic else "llm_analysis") / "llm_analysis_upload.json"
            upload_json.parent.mkdir(parents=True, exist_ok=True)
            with open(upload_json, "w", encoding="utf-8") as f:
                json.dump(trimmed, f, ensure_ascii=False, indent=2)
            logger.info(
                "上传版 analysis 已保存: %s（可见文字 %d，上限 %d，已丢弃 %d 个单元：%s）",
                upload_json, upload_chars, UPLOAD_CHAR_LIMIT, len(dropped_units),
                "、".join(dropped_units) if dropped_units else "无",
            )

            # ========== Pass B：上传版 ==========
            upload_parts = self._build_parts(
                trimmed,
                [('gzh', gzh_impl), ('bili', bili_impl)],
                author, title, bvid, rendering_dir,
                bili_impl=bili_impl,
                economic=economic,
            )

            gzh_html = GongzhonghaoRendererImpl.markdown_to_html(upload_parts['gzh'])
            gzh_file = rendering_dir / "summary_gongzhonghao.html"
            with open(gzh_file, "w", encoding="utf-8-sig") as f:
                f.write(gzh_html)
            logger.info("公众号版（上传）已保存: %s", gzh_file)

            bili_file = rendering_dir / "summary_bilibili.md"
            with open(bili_file, "w", encoding="utf-8-sig") as f:
                f.write(upload_parts['bili'])
            logger.info(
                "B站版（上传）已保存: %s%s",
                bili_file, "" if bvid else "（无 bvid，时间戳不带跳转链接）",
            )

            return md_summary

    def _render_tldr_section_parallel(self, renderers: list[tuple[str, BaseRenderer]], renderer_parts: dict[str, list[str]], tldr: dict, rendering_dir: Path, bili_impl: BilibiliRendererImpl | None, *, economic: bool = False) -> None:
        """渲染精炼总结章节"""
        image_paths = {}

        if bili_impl is not None:
            image_paths = bili_impl.render_tldr_tables(tldr, rendering_dir)
        ...
        if short_ops:
            for name, renderer in renderers:
                ...
                if not short_ops:
                    ...
                    continue
                image_path = image_paths.get("short_ops_image_path") if name == 'bili' else None
                if image_path:
                    renderer_parts[name].append(f"![短期机会速览]({image_path.name})\n\n")
                else:
        ...
        if opinion_rows:
            for name, renderer in renderers:
                ...
                if not opinion_rows:
                    renderer_parts[name].append("无\n\n")
                    continue
                image_path = image_paths.get("opinion_image_path") if name == 'bili' else None
                if image_path:
                    renderer_parts[name].append(f"![个股观点速览]({image_path.name})\n\n")
                else:
        ...
        if event_rows:
            for name, renderer in renderers:
                ...
                if not event_rows:
                    renderer_parts[name].append("无\n\n")
                    continue
                image_path = image_paths.get("event_image_path") if name == 'bili' else None
                if image_path:
                    renderer_parts[name].append(f"![个股事件速览]({image_path.name})\n\n")
                else:
        ...
        if knowledge:
            for name, renderer in renderers:
                ...
                if not knowledge:
                    renderer_parts[name].append("无\n\n")
                    continue
                image_path = image_paths.get("knowledge_image_path") if name == 'bili' else None
                if image_path:
                    renderer_parts[name].append(f"![市场知识索引]({image_path.name})\n\n")
                else:
        ...
        if tips:
            for name, renderer in renderers:
                ...
                if not tips:
                    renderer_parts[name].append("无\n\n")
                    continue
                image_path = image_paths.get("tips_image_path") if name == 'bili' else None
                if image_path:
                    renderer_parts[name].append(f"![交易技巧摘要]({image_path.name})\n\n")
                else:
