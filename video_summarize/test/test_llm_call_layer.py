...
class TestProfileStageResolution:
    def test_default_profile_keeps_max_thinking(self) -> None:
        ...
        注意 effort 现在按阶段分级（见 config/llm_profiles.json 的 stages 表），
    ...
    def test_qwen_profile_translation(self) -> None:
        profile = resolve_profile("qwen3.8-flash")
        assert profile["provider"] == "dashscope"
        assert profile["kv_user_id"] is False
