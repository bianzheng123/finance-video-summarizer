...
def install_logging_hooks() -> None:
    """安装记录工厂与线程上下文传播补丁（不挂任何 handler）。

    供桌面 GUI 等非 CLI 入口复用：GUI 不想调用 :func:`setup_logging`（其 FileHandler
    会写到 ``_MEIPASS/logs``），但仍需要 ``record.step`` / ``record.session`` 字段与
    线程池上下文传播，故把这两件幂等安装动作单独暴露。两次安装均带幂等标记，可安全重复调用。
    """
